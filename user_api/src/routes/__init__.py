...
def register_public_routes(app: Blueprint) -> None:
    ...
    apidoc.static_url_path = "/api/users/swaggerui"

    # Create a Blueprint to register routes
    bp = Blueprint("USERS_API", __name__, url_prefix="/api/users",
                   static_url_path='/api/users/swaggerui')
    api = Api(
        bp,
        version="1.0",
        title="Users Service API",
        description="API documentation for the Users Service of the Distributed Systems project",
        doc='/'
    )
