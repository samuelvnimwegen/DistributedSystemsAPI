...
def register_public_routes(app: Blueprint) -> None:
    ...
    apidoc.static_url_path = "/api/movies/swaggerui"

    # Create a Blueprint to register routes
    bp = Blueprint("MOVIES_API", __name__, url_prefix="/api/movies",
                   static_url_path='/api/movies/swaggerui')
    api = Api(
        bp,
        version="1.0",
        title="Movies Service API",
        description="API documentation for the Movie Service of the Distributed Systems project",
        doc='/'
    )
